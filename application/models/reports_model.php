...
class reports_model extends CI_Model {
	...
	public function get_report_activities($start_date, $end_date, $category_id, $audience, $focus, $model, $dealership_id){
		$filters = " ";
		$filter_audience = " ";
		$filter_focus = " ";
		$filter_model = " ";
		...
		$result = $this->db->query("
				...
				SELECT d.name as dealership, MONTHNAME(a.start_date) AS month_date, a.name, ca.description, DATE_FORMAT(a.start_date,'%d/%m/%Y') as start_date, a_c.audiences, a_f.focus, a.expense, a_m.models, category, metric, quantity
				FROM activities a 
					...
					LEFT JOIN (SELECT activity_id, GROUP_CONCAT(cat.description SEPARATOR '|') as 'category', GROUP_CONCAT(met.description SEPARATOR '|') as 'metric', GROUP_CONCAT(quantity SEPARATOR '|') AS 'quantity'
								FROM activity_metrics ame
									INNER JOIN categories cat ON ame.category_id = cat.id
				...
				" GROUP BY (a.start_date), a.name, ca.description, DATE_FORMAT(a.start_date, '%d/%m/%Y'), a_c.audiences,
    						a_f.focus, a.expense, a_m.models, category, metric,quantity 
